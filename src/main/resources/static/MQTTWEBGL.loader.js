function createUnityInstance(canvas, config, onProgress) {
  ...
  function errorListener(e) {
    ...
    if (
      ...
      !Module.stackTraceRegExp.test(message)
    )
      return;

    var filename =
      e.filename || (error && (error.fileName || error.sourceURL)) || '';
  ...
  var Module = {
    ...
    cacheControl: function (url) {
      return url == Module.dataUrl || url.match(/\.bundle/)
        ? 'must-revalidate'
        : 'no-store';
    },
    ...
    printErr: function (message) {
      ...
      if (
        ...
        message.indexOf('wasm streaming compile failed') != -1
      ) {
  ...
  Module.streamingAssetsUrl = new URL(
  ...
  Module.SystemInfo = (function () {
    ...
    if (browser == 'Safari')
      browserVersion = extractRe('Version/(.*?) ', ua, 1);
    if (browser == 'Internet Explorer')
      browserVersion = extractRe('rv:(.*?)\\)? ', ua, 1) || browserVersion;
      ...
      if (gl) {
        gpu =
          (gl.getExtension('WEBGL_debug_renderer_info') &&
            gl.getParameter(
              0x9246 /*debugRendererInfo.UNMASKED_RENDERER_WEBGL*/
            )) ||
    ...
    return {
      ...
      hasFullscreen:
        ...
        !!document.body.webkitRequestFullscreen, // Safari still uses the webkit prefixed version
  ...
  function errorHandler(message, filename, lineno) {
    ...
    if (message.indexOf('fullscreen error') != -1) return;
  ...
  function progressUpdate(id, e) {
    ...
    var totalProgress = started
      ? (started -
          unfinishedNonComputable -
          (total ? (computable * (total - loaded)) / total : 0)) /
        started
      : 0;
  ...
  Module.readBodyWithProgress = (function () {
    ...
    function readBodyWithProgress(response, onProgress, enableStreaming) {
      ...
      var lengthComputable =
        typeof response.headers.get('Content-Length') !== 'undefined';
      var estimatedContentLength = estimateContentLength(
        response,
        lengthComputable
      );
  ...
  Module.UnityCache = (function () {
          ...
          return new Promise(
            function (resolve, reject) {
              try {
                ...
                var accessMode =
                  ['put', 'delete', 'clear'].indexOf(operation) != -1
                    ? 'readwrite'
                    : 'readonly';
    ...
    UnityCache.prototype.getMetaDataEntries = function () {
      ...
      return new Promise(function (resolve, reject) {
        var transaction = self.database.transaction(
          [RequestMetaDataStore.name],
          'readonly'
        );
  ...
  Module.cachedFetch = (function () {
    ...
    function isCacheEnabled(url, init) {
      ...
      if (
        init &&
        ['must-revalidate', 'immutable'].indexOf(init.control) == -1
      ) {
        return false;
      }
      ...
      function fetchAndStoreInCache(resource, init) {
        return fetch(resource, init).then(function (response) {
          ...
          if (response.status === 304) {
            ...
            unityCache
              .updateRequestMetaData(cache.metaData)
              .then(function () {
                log(
                  "'" +
                    cache.metaData.url +
        ...
        .then(function (result) {
          ...
          if (cache.control == 'immutable') {
            ...
            return fetch(url, { method: 'HEAD' }).then(function (headResult) {
              cache.revalidated = ['Last-Modified', 'ETag'].every(function (
                header
              ) {
                return (
                  !response.headers.get(header) ||
                  response.headers.get(header) == headResult.headers.get(header)
                );
              });
      ...
      dataPromise.then(function (data) {
        ...
        if (
          !String.fromCharCode.apply(
            null,
            data.subarray(pos, pos + prefix.length)
          ) == prefix
        )
  ...
  return new Promise(function (resolve, reject) {
    if (!Module.SystemInfo.hasWebGL) {
      ...
      if (
        Module.SystemInfo.browser == 'Safari' &&
